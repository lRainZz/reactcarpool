            ...
            snap.forEach(async function(childSnapshot) {
              ...
              if (password == snap.child(SnapshotKey).child("Password").val()){
                ...
                .then(async function(OptionsSnap) 
                {
                  ...
                  .then((UserSnap) =>
                  {
                    Promise.all(UserSnap.forEach((childUserSnap) => {
                      let Ukey = childUserSnap.child('key').val();
                      let UEmail = childUserSnap.child('Email').val();
                      let UFullName = childUserSnap.child('FullName').val();
                      let UPassword = childUserSnap.child('Password').val();
                      let UZipCode = childUserSnap.child('ZipCode').val();
                                        ...
                                        if (childSnapshot2.child('Creator').val() == '1'){
                                          ...
                                          .then((snapshot3) =>
                                          {
                                            ...
                                            .then((snapshot4) =>
                                            {
                                              ...
                                              {
                                                GLOBALS.Invites = Export;
                                                console.log(GLOBALS.Invites)
                                              }
                                            });
                                          });
                                        }
